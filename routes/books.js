...
router.post("/", async function (req, res, next) {
  try {
    const validateSchema = validate(req.body, bookSchema);
    if(!validateSchema.valid){
      return next({status:404, err: validateSchema.errors.map(e => e.stack)})
    }
...
router.put("/:isbn", async function (req, res, next) {
  try {
        const validateSchema = validate(req.body, updateBookSchema)
    if(!validateSchema.valid){
      return next({status:404, err: validateSchema.errors.map(e => e.stack)})
    }
